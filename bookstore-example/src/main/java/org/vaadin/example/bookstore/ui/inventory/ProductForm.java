...
public class ProductForm extends Div {
    ...
    public ProductForm(InventoryViewLogic sampleCrudLogic) {
        ...
        binder.forField(price).withConverter(new PriceConverter()) //convert from String to data type
                .bind("price"); // bind to the UI and the data model Product
        binder.forField(stockCount).withConverter(new StockCountConverter())
                .bind("stockCount");
        binder.bindInstanceFields(this); // bind the remaining fields
